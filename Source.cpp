...
void numDataPoints(string fileName, int &numTemp, int &numWSpeed)
{
	...
	while (!inFile.eof())
	{
		...
		if (charac=='\n')
		{
			...
			else if (counter>numTemp)
			{
				cout << "Number Of Temperature Drop Values  In Line " << i+2 << " Is Greater Than Number Of Temperature Values In The First Line\n";
				counter=0;
				exit(-1);
			}
			else if (counter<numTemp)
			{
				cout << "Number Of Temperature Drop Values In Line " << i+2 << " Is Less Than Number Of Temperature Values In The First Line\n";
				counter=0;
				exit(-1);
			}
			i++;
	...
	if (numTempDropLines<numWSpeed || numTempDropLines>numWSpeed)
	{
		cout << "Number of lines of temperature drop values does not match the number values in the second line";
		exit(-1);
	}
